      ...
      {ONBOARDING_CATEGORIES.map((c) => {
        ...
        return (
          <div
            ...
            className={`press-tactile relative cursor-pointer rounded-2xl border-2 p-3.5 transition-all ${
              isSelected
                ? "shadow-primary-500/15 border-[var(--color-brand)] bg-[var(--color-brand-light)] shadow-md"
          ...
          >
            ...
            <div
              className={`mb-2.5 flex h-9 w-9 items-center justify-center rounded-xl border ${
                isSelected
            ...
            <div className="text-xs font-extrabold text-[var(--color-text)]">{c.label}</div>
            <div className="mt-0.5 line-clamp-2 text-[10px] leading-snug font-medium text-[var(--color-text-3)]">
              {c.desc}
            </div>
          </div>
        );
